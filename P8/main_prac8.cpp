...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	t_BlackHole.LoadTGA("BlackHole.tga");
	t_BlackHole.BuildGLTexture();
	t_BlackHole.ReleaseImage(); //Metodo convenienente pero no necesario
...
void prisma (GLuint textura1, GLuint textura2)  //Funcion creacion prisma
{
	...
	GLfloat vertice [8][3] = {
		...
		glBindTexture(GL_TEXTURE_2D,t_BlackHole.GLindex);   // choose the texture to use.
...
void display ( void )   // Creamos la funcion donde se dibuja
{
	...
	glTranslatef(pos_camX, pos_camY, pos_camZ);
		...
		glBindTexture(GL_TEXTURE_2D, t_BlackHole.GLindex);

		glBegin(GL_QUADS);
		glTexCoord2f(0.8f, 0.55f); glVertex3f(0.0f, 0.0f, 0.0f); //Vertice 0
		glTexCoord2f(1.0f, 0.55f); glVertex3f(7.0f, 0.0f, 0.0f); //Vertice 1
		glTexCoord2f(2.0f, 2.0f); glVertex3f(7.0f, 5.0f, 0.0f); //Vertice 2
		glTexCoord2f(0.0f, 2.0f); glVertex3f(0.0f, 5.0f, 0.0f); //Vertice 3

		glEnd();
		

	//	prisma(t_BlackHole.GLindex, t_Ajedrez1.GLindex);
...
int main ( int argc, char** argv )   // Main Function
{
  ...
  glutCreateWindow    ("Practica 8 2020-1"); // Nombre de la Ventana
