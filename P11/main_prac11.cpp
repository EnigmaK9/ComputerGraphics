...
float movBrazoDerechoInc = 0.0f;
float movBrazoIzquierdo = 0.0f;
float movBrazoIzquierdoInc = 0.0f;
float rotRodDer = 0.0f; 
float rotRodDerInc = 0.0f ;
float rotInc = 0.0f;
// tienen dos funcoines, para manipular al personaje. Afecta a esas variables de dibujo. Calculando la interpolación, se ven reflejadas en el escenario.
#define MAX_FRAMES 15
int i_max_steps = 90; //cuadros que se calculan entre cada transicion
int i_curr_steps = 0;
...
typedef struct _frame
{
	...
	float movBrazoIzquierdo;     //esta es de la estructura y arriba es la de dibujo.
	float movBrazoIzquierdoInc;
...
void saveFrame ( void )
{
	...
	printf("frameindex %d\n",FrameIndex);			 //comenzar animacion desde cero

	KeyFrame[FrameIndex].posX=posX;   //variables que se mandan a la estructura
	KeyFrame[FrameIndex].posY=posY;	  //aquí están todas las variables
	KeyFrame[FrameIndex].posZ=posZ;

	KeyFrame[FrameIndex].rotRodIzq=rotRodIzq;  // para regresar el monito a su posicion
	...
	//movimiento del brazo izquierdo
	KeyFrame[FrameIndex].movBrazoIzquierdo = movBrazoIzquierdo; 
...
void resetElements(void)
{
	...
	movBrazoIzquierdo = KeyFrame[0].movBrazoIzquierdo;


}

void interpolation ( void ) //operacion del pizarron. Calcula los incrementos necesarios. del lazo izquierdo es asignacion, se encuentra el incremento
{
	KeyFrame[playIndex].incX = (KeyFrame[playIndex + 1].posX - KeyFrame[playIndex].posX) / i_max_steps;	 //cantidad de cuadros intermedios que tengo
	...
	//Brazo izquierdo
	KeyFrame[playIndex].movBrazoIzquierdoInc = (KeyFrame[playIndex + 1].movBrazoIzquierdo - KeyFrame[playIndex].movBrazoIzquierdo) / i_max_steps;
	...
	glPushMatrix();//Pecho
		...
		glPushMatrix(); //Brazo derecho-->
			...
			glPushMatrix();
				...
				glRotatef(-45-movBrazoDerecho, 0, 1, 0); 
		...
		glPushMatrix(); //Brazo izquierdo <--
			...
			glPushMatrix();
				glTranslatef(-0.25, 0, 0);
				glRotatef(45, 0, 1, 0); 
				glRotatef(25+movBrazoIzquierdo, 0, 0, 1);
				glRotatef(25, 1, 0, 0); 
				glTranslatef(-0.75, 0, 0);
				fig7.prisma(0.7, 1.5, 0.7, 0);
			glPopMatrix();
...
void animacion()
{
	...
	{		
		...
		else
		{
			...
			movBrazoIzquierdo = KeyFrame[playIndex].movBrazoIzquierdoInc;
			i_curr_steps++;
		}
...
int main ( int argc, char** argv )   // Main Function
{
  ...
  glutCreateWindow    ("Practica 11 20201"); // Nombre de la Ventana
